...
resource "null_resource" "copy_files" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo 'attempts=0' >> /root/start-nats.sh",
      ...
      "echo '    echo \"nats-server not found. Waiting for $wait_time seconds...\"' >> /root/start-nats.sh",
      "echo '    sleep $wait_time' >> /root/start-nats.sh",
      "echo '    attempts=$((attempts + 1))' >> /root/start-nats.sh",
      "echo 'done' >> /root/start-nats.sh",
      "echo 'echo \"nats-server is available.\"' >> /root/start-nats.sh",
      "echo 'hostname=$(cat /etc/hostname)' >> /root/start-nats.sh",
      ...
      "echo 'check_docker() {' >> /root/start-docker.sh",
      "echo '    command -v docker >/dev/null 2>&1' >> /root/start-docker.sh",
      "echo '}' >> /root/start-docker.sh",
      "echo 'attempts=0' >> /root/start-docker.sh",
      "echo 'max_attempts=2' >> /root/start-docker.sh",
      "echo 'wait_time=30' >> /root/start-docker.sh",
      "echo 'while ! check_docker; do' >> /root/start-docker.sh",
      "echo '    if [ $attempts -ge $max_attempts ]; then' >> /root/start-docker.sh",
      "echo '        echo \"Docker is not installed or running. Installing Docker...\"' >> /root/start-docker.sh",
      "echo '        apt update -qq' >> /root/start-docker.sh",
      "echo '        apt install -qq -y docker.io' >> /root/start-docker.sh",
      "echo '        if ! check_docker; then' >> /root/start-docker.sh",
      "echo '            echo \"Docker installation failed. Exiting.\"' >> /root/start-docker.sh",
      "echo '            exit 1' >> /root/start-docker.sh",
      "echo '        fi' >> /root/start-docker.sh",
      "echo '        echo \"Docker successfully installed.\"' >> /root/start-docker.sh",
      "echo '        break' >> /root/start-docker.sh",
      "echo '    fi' >> /root/start-docker.sh",
      "echo '    echo \"Docker not found. Waiting for $wait_time seconds...\"' >> /root/start-docker.sh",
      "echo '    sleep $wait_time' >> /root/start-docker.sh",
      "echo '    attempts=$((attempts + 1))' >> /root/start-docker.sh",
      "echo 'done' >> /root/start-docker.sh",
      "echo 'echo \"Docker is available.\"' >> /root/start-docker.sh",
